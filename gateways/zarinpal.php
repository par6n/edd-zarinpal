    ...
    class EDD_ZarinPal_Gateway {
        ...
        public function process( $purchase_data ) {
            ...
            if ( $payment ) {
                ...
                if ( $result['data']['code'] == 100 ) {
                    edd_insert_payment_note( $payment, 'کد تراکنش زرین‌پال: ' . $result['data']["authority"] );
                    edd_update_payment_meta( $payment, 'zarinpal_authority', $result['data']["authority"] );
                    $_SESSION['zp_payment'] = $payment;

                    wp_redirect( sprintf( $redirect, $result['data']["authority"] ) );
                } else {
                    edd_insert_payment_note( $payment, 'کد خطا: ' . $result['errors']['code'] );
                    edd_insert_payment_note( $payment, 'علت خطا: ' . $this->error_reason( $result['errors']['code'] ) );
                    edd_update_payment_status( $payment, 'failed' );

                    edd_set_error( 'zarinpal_connect_error', 'در اتصال به درگاه مشکلی پیش آمد. علت: ' . $this->error_reason( $result['errors']['code'] ) );
                    edd_send_back_to_checkout();
                }
            } else {
        ...
        public function verify() {
            ...
            if ( isset( $_GET['Authority'] ) ) {
                ...
                if ( $result['data']['code'] == 100 ) {
                    edd_insert_payment_note( $payment->ID, 'شماره تراکنش بانکی: ' . $result ['data']['ref_id'] );
                    edd_update_payment_meta( $payment->ID, 'zarinpal_refid', $result ['data']['ref_id'] );
